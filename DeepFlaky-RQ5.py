...
# 新增：语义感知的数据增强函数
def semantic_structural_augmentation(original_df, augmentation_factor=2, flaky_boost=1.5):
    """
...
def predict_RF_perProject(data, project_name, balance, classifier, mintree, Features_type, ig, result_by_test_name,
    ...
    for target_project in all_projects:
        ...
        # 数据增强处理（只增强训练数据）
        if use_augmented:
            print(f"Applying data augmentation for training data (excluding {target_project})...")
            augmented_train_data = semantic_structural_augmentation(
                train_data,
                augmentation_factor=3,
                flaky_boost=2.0
            )
            print(f"Training data augmented from {len(train_data)} to {len(augmented_train_data)} samples")
        else:
            augmented_train_data = train_data

        # 准备训练和测试数据
        x_train = augmented_train_data.drop(['flakyStatus', 'test_name', 'project'], axis=1)
        y_train = augmented_train_data['flakyStatus']
        ...
        if use_supcon:
            try:
                supcon_model, scaler = train_supcon_model(x_train.values, y_train, x_train.shape[1])
                x_train_supcon = extract_supcon_features(supcon_model, scaler, x_train.values)
                x_test_supcon = extract_supcon_features(supcon_model, scaler, x_test.values)
        ...
        for i in range(len(test_names_as_list)):
            new_row = pd.DataFrame(
                [["PerProject", balance, ig, mintree, classifier, Features_type, test_names_as_list[i],
        ...
        try:
            ...
            auc_scores_total.append(auc_score)
        except:
            auc_scores_total.append(0)
    ...
    return TN_total, FP_total, FN_total, TP_total, round((Precision * 100)), round(((Recall) * 100)), round(
        (F1 * 100)), round((avg_auc * 100)), result_by_test_name
...
if __name__ == '__main__':
    ...
    for ig in minIGList:
        ...
        if ig != 0:
            keep_columns = keep_minIG + ['flakyStatus', 'test_name', 'project'] + semantic_columns
        ...
        for mintree in treeSize:
            for bal in balance:
                for cl in classifier:
                    ...
                    TN, FP, FN, TP, Precision, Recall, f1, auc_score, result_by_test_name = predict_RF_perProject(
                        combined_data, 'project', bal, cl, mintree, "Combined-Features", ig, result_by_test_name,
                        use_supcon=use_supcon, use_augmented=use_augmented)

                    new_row = pd.Series(["PerProject", "PerProject", bal, mintree, "Combined-Features", ig,
                                         combined_data.shape[1] - 3, cl, TP, FN, FP, TN, Precision, Recall, f1,
                                         auc_score],
                                        index=result.columns)
                    result = pd.concat([result, new_row.to_frame().T], ignore_index=True)

                    # 保存结果
                    result_by_test_name.to_csv(output_dir + "IG_" + str(ig) + '/prediction_result_per_test.csv',
                                               index=False)
                    ...
                    confusion_matrix_by_project.to_csv(
                        output_dir + "IG_" + str(ig) + '/prediction_result_by_project.csv', index=False)
