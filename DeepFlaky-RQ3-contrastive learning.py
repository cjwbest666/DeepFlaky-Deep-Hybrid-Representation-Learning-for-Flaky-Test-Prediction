                ...
                for bal in balance:
                    for cl in classifier:
                        ...
                        TN, FP, FN, TP, Precision, Recall, f1, auc_score, result_by_test_name = predict_RF_crossValidation(
                            only_processed_data, k, fold, bal, cl, "Flake-Flagger-Features", ig,
                            result_by_test_name, use_supcon=use_supcon, use_augmented=use_augmented)
                        new_row = pd.Series(["CrossAllProjects", fold, bal, "Flake-Flagger-Features", ig,
                                             only_processed_data.shape[1] - 1, cl, TP, FN, FP, TN, Precision, Recall,
                                             f1, auc_score],
                                            index=result.columns)
                        result = pd.concat([result, new_row.to_frame().T], ignore_index=True)
                        print(
                            f"TP: {TP}, FN: {FN}, FP: {FP}, TN: {TN}, Precision: {Precision}, Recall: {Recall:.4f}, f1: {f1:.4f}, AUC: {auc_score:.4f}")

                        # 2. 仅使用语义表征特征
                        only_semantic_data = vocabulary_processed_data_full[
                            semantic_columns + ['flakyStatus', 'test_name']]
                        TN, FP, FN, TP, Precision, Recall, f1, auc_score, result_by_test_name = predict_RF_crossValidation(
                            only_semantic_data, k, fold, bal, cl, "Semantic-Features", ig, result_by_test_name,
                            use_supcon=use_supcon, use_augmented=use_augmented)
                        new_row = pd.Series(["CrossAllProjects", fold, bal, "Semantic-Features", ig,
                        ...
                        TN, FP, FN, TP, Precision, Recall, f1, auc_score, result_by_test_name = predict_RF_crossValidation(
                            combined_data, k, fold, bal, cl, "Combined-Features", ig, result_by_test_name,
                            use_supcon=use_supcon, use_augmented=use_augmented)

                        new_row = pd.Series(["CrossAllProjects", fold, bal, "Combined-Features", ig,
